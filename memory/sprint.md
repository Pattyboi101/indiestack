...
### Search Quality (Step 1)
- Added 22 new `_CAT_SYNONYMS` entries (1934 → 1956 unique keys, 0 duplicates; also removed 2 pre-existing duplicates):
  - **Developer**: `codemod` → developer — AST-based code transformation (jscodeshift, ast-grep)
  - **Developer**: `ast` → developer — Abstract Syntax Tree tools (babel-parser, acorn)
  - **Developer**: `jscodeshift` → developer — Facebook's AST codemod runner (9k★)
  - **Developer**: `ast-grep` → developer — structural AST search/rewrite in Rust (7k★)
  - **CLI**: `homebrew` → cli — missing macOS/Linux package manager (42k★)
  - **Documentation**: `typedoc` → documentation — TypeScript API doc generator (7k★)
  - **Documentation**: `jsdoc` → documentation — JS documentation standard/generator (14k★)
  - **Documentation**: `compodoc` → documentation — Angular documentation generator (4k★)
  - **Testing**: `ava` → testing — concurrent Node.js test runner (20k★)
  - **Testing**: `nock` → testing — HTTP mock/interceptor for Node.js (12k★)
  - **Testing**: `supertest` → testing — HTTP assertion library for Node.js (13k★)
  - **Testing**: `testcafe` → testing — cross-browser E2E testing (10k★)
  - **Testing**: `nightwatch` → testing — Selenium/WebDriver E2E (11k★)
  - **DevOps**: `compose` → devops — Docker Compose standalone keyword
  - **DevOps**: `bpf` → devops — Berkeley Packet Filter (complement to ebpf→devops)
  - **DevOps**: `oci` → devops — OCI container image/runtime queries
  - **Monitoring**: `cloudwatch` → monitoring — AWS CloudWatch alternative queries
  - **API**: `apidog` → api — Postman-alternative API design platform
  - **API**: `openapi-generator` → api — OpenAPI client/server codegen
  - **Database**: `nhost` → database — Hasura-based Firebase alternative (8k★)
  - **Frontend**: `wasm-bindgen` → frontend — Rust/WASM JavaScript bindings
  - **Background**: `benthos` → background — Redpanda Connect stream processor (8k★)
- Removed 2 pre-existing duplicate keys: `msw` (kept newer entry at line ~5160), `opa` (kept newer entry at line ~5151)

### Catalog Script (Step 2)
- Added 5 new tools to `scripts/add_missing_tools.py` (519 → 524 unique):
  - **TestCafe** (DevExpress/testcafe, 10k★) — cross-browser E2E testing; testing-tools
  - **OXLint** (oxc-project/oxc, 5k★) — Rust-based JS/TS linter 50-100× faster than ESLint; testing-tools
  - **Moto** (getmoto/moto, 8k★) — mock AWS services in Python tests; testing-tools
  - **Typebot** (baptisteArno/typebot.io, 10k★) — open-source conversational form builder; forms-surveys
  - **Benthos** (redpanda-data/connect, 8k★) — declarative stream processor; background-jobs
