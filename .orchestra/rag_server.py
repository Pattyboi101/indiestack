...
LLM: passthrough by default (instant, no Ollama needed for queries)
     Set RAG_USE_OLLAMA=1 for real entity extraction (slow on CPU)

Queries use aquery_data() — returns raw chunks, no LLM synthesis.
Claude agents do their own reasoning from the chunks.

Run:  python3 .orchestra/rag_server.py          (stdio transport)
"""
...
RAG_DIR = str(pathlib.Path.home() / "indiestack" / ".rag")
LLM_MODEL = os.environ.get("RAG_LLM_MODEL", "qwen2:1.5b")
...
async def _passthrough_llm(prompt, **kwargs):
    ...
    For real entity extraction (builds the knowledge graph):
        RAG_USE_OLLAMA=1 python3 .orchestra/rag_server.py
    """
    return "<|COMPLETE|>"
...
async def rag_query(
    ...
    """Search the shared IndieStack knowledge base.
    ...
    try:
        ...
        # Hard truncate to max_tokens (4 chars per token rough estimate)
...
async def rag_store(content: str, tags: str = "") -> str:
    ...
    Use this to persist gotchas, decisions, patterns, and CEO verdicts.
