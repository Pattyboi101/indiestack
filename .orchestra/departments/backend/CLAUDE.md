...
- Space-separated compound keys (e.g. "load balancer": "devops") ARE supported — the routing checks bigrams (adjacent token pairs) before individual tokens at each position. This means "load balancer tool" correctly routes via "load balancer"→devops instead of "load"→testing. Always prefer a compound entry when a single token would collide with a different category. Update `route_query()` in `scripts/test_search_routing.py` when adding new compound keys (the simulator mirrors db.py logic).
