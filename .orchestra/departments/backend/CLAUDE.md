...
- `_CAT_SYNONYMS` uses short-name values — NOT category slugs. Key mappings: frontend-frameworks → "frontend", design-creative → "design", file-storage → "file", background-jobs → "background", devops-infrastructure → "devops", developer-tools → "developer", ai-automation → "ai", mcp-servers → "mcp". The "customer" short-name maps to live-chat tools, "support" maps to helpdesk tools.
