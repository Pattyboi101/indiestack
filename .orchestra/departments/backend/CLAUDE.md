...
- Probe pattern 68 (May 2026): time-tracking / team-chat / session-store / bot-builder / boilerplate-bigram dead zones. "time tracking tool" → api (wrong; "time"→api was reinforcing real-time routing) — changed "time"→project since "real"→api at first position already covers real-time; guard bigrams "time series"/"time database"→database protect DB routing. "billable hours" → raw_first; "billable"→invoicing added. "team chat app" → customer (live-chat collision); bigram "team chat"→developer. "slack/mattermost/telegram alternative" → raw_first (stop: alternative); bare tokens→developer. "toggl alternative" → raw_first; "toggl"→project. "discord bot"/"slack bot"/"telegram bot" → raw_first; bigrams→developer. "session store redis" → auth ("session"→auth fired first); bigram "session store"→caching overrides. "ai standard specification" → api ("specification"→api); bigram "ai standard"→standard (singular). "fullstack starter/template" → frontend ("fullstack"→frontend at i=0); bigrams "fullstack starter"/"fullstack template"→boilerplate. 22 new tests. Now 395/395 routing tests pass.
