...
- Probe pattern 36 (May 2026): binary serialization / content moderation / SVG / address dead zones. "protocol buffers" (plural) misrouted via "protocol"→mcp; added plural bigram + "protobufs"→api. "profanity"/"toxicity" unmapped → raw_first; both added → ai. "svg" unmapped → raw_first; added →frontend. "address validation/autocomplete/lookup" misrouted via "validation"→developer; added bigrams →maps. Also removed pre-existing duplicate key "msgpack" (was at two locations). Now 863/863 routing tests pass.
- Probe pattern 37 (May 2026): cross-platform / token counter / content safety / PII dead zones. "cross platform" loses "platform" to stop-word stripping → raw_first "cross"; added "cross"→frontend + "cross origin"→api bigram (CORS queries). "token counter" misrouted to auth via bare "token"→auth; bigram "token counter"→ai. "content safety"/"content filter"/"content filtering" misrouted to CMS via "content"→cms; three bigrams added →ai. "pii"/"pii detection"/"pii masking" all raw_first; added →security. Now 882/882 routing tests pass.
