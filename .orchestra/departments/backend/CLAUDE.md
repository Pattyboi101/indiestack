...
- Space-separated compound keys (e.g. `"load balancer": "devops"`) ARE supported — routing checks bigrams (adjacent token pairs) BEFORE individual tokens at each position. Prefer a compound entry when a single token would collide with a different category. After adding, run `scripts/test_search_routing.py` (727 tests as of May 2026 — check output for current count). Also run `scripts/validate_synonyms.py` to catch duplicate keys (Python dicts silently override with last value) and dead 3-token keys (router only builds bigrams — 3-token compound keys can never fire). The test file docstring contains 27 probe patterns for finding routing gaps systematically.
- Probe pattern 47 (May 2026): sales-tax misroute + named tax-tool dead zones. "sales tax api"/"sales tax calculation" routed to crm via bare "sales"→crm token (wrong; Anrok/TaxJar/Avalara belong in Invoicing). "avalara" and "taxjar" had no mapping → raw_first fired. Fixed: bigram "sales tax"→invoicing (fires before "sales"→crm); bare "avalara","taxjar"→invoicing. Regressions guarded: bare "tax compliance"→invoicing, "vat compliance"→invoicing, "sales pipeline crm"→crm all unaffected. Now 727/727 routing tests pass.
- Dot-domain suffix forms (e.g. `"make.com"`, `"cal.com"`, `"sentry.io"`) must be added explicitly — the router does NOT strip TLD suffixes. Pattern 27 added entries for 15 high-traffic tools. When adding a new tool with a well-known domain, add both the bare name AND the dot-domain form.
