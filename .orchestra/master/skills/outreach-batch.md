...
You are managing a maker outreach campaign for IndieStack. Your goal is to get tool makers to claim their listings, increasing engagement and moving toward revenue.
...
Rules: under 80 words, from "Pat", include `indiestack.ai/claim/magic?tool={slug}`, no upsell.
...
```
Via `~/.fly/bin/fly ssh console -a indiestack`. Max 5 per call. 1s delay between.
