...
async def admin_post(request: Request):
    ...
    if "action" in form:
        ...
        if action == "approve_all":
            ...
            await update_tool_status(request.state.db, tool_id_int, status)
            if status == "approved":
                # ── Round 9: Competitor pings + approval notification ──
                try:
                    approved_tool = await get_tool_by_id(request.state.db, tool_id_int)
                    if approved_tool:
                        ...
                        competitors = await get_makers_in_category(request.state.db, cat_id, exclude_tool_id=tool_id_int)
