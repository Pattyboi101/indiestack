...
def footer_html() -> str:
    ...
    <form action="/api/subscribe" method="POST" style="display:flex;gap:8px;justify-content:center;max-width:400px;margin:0 auto;">
        ...
        <button type="submit" style="padding:8px 18px;border-radius:999px;border:none;
          ...
          <span style="color:rgba(255,255,255,0.7);font-size:13px;font-weight:600;display:block;width:100%;text-align:center;margin-bottom:12px;">Makers keep 92&ndash;94% of every sale.</span>
...
def featured_card(featured: dict) -> str:
    ...
    return f"""
    <div class="card" style="background:linear-gradient(135deg, #0F1D30 0%, #1A2D4A 50%, #2B4A6E 100%);
                border:none;padding:32px;color:white;position:relative;overflow:hidden;
                box-shadow:0 8px 32px rgba(15,29,48,0.3);">
        <div style="position:absolute;top:-20px;right:-20px;font-size:120px;opacity:0.08;color:var(--gold);">&#9733;</div>
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
            <span style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;
                         color:#FDE68A;background:rgba(253,230,138,0.2);padding:4px 12px;border-radius:999px;
        ...
        <h3 style="font-family:var(--font-display);font-size:24px;margin-bottom:8px;">
            <a href="/tool/{slug}" style="color:white;">{headline or name}</a>
        </h3>
        <p style="color:rgba(255,255,255,0.85);font-size:15px;margin-bottom:20px;line-height:1.6;">{desc or tagline}</p>
        <a href="/tool/{slug}" class="btn" style="background:#FDE68A;color:#1A2D4A;font-weight:700;
