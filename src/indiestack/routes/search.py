...
async def search(request: Request):
    ...
    if not results:
        ...
        if query.strip():
            ...
            <p style="color:var(--ink-muted);font-size:16px;margin-bottom:24px;">
                No developer tools found for that query. Try different keywords or
        ...
        """
        return HTMLResponse(page_shell(title="Search", body=body + email_sticky_bar(), user=user, description="Search 3,100+ developer tools. Find the right indie tool for auth, payments, analytics, databases, and more."))
    ...
    if query.strip() and results:
        ...
        if is_alternatives:
            ...
            <div style="background:var(--cream-dark);border:1px solid var(--border);border-left:3px solid var(--accent);
                ...
                Looking for <strong>{safe_query}</strong>? These developer tools are alternatives you can use instead.
    ...
    """
    desc = f"Search results for '{safe_query}' — {total} developer tools found on IndieStack." if query else "Search 3,100+ developer tools. Find the right indie tool for auth, payments, analytics, databases, and more."
