...
def signup_form(error: str = "", values: dict = None, next_url: str = "") -> str:
    v = values or {}
    alert = f'<div class="alert alert-error">{escape(error)}</div>' if error else ''
...
async def signup_get(request: Request):
    if request.state.user:
        return RedirectResponse(url="/dashboard", status_code=303)
    next_url = _safe_next(request.query_params.get("next", ""))
    return HTMLResponse(page_shell("Sign Up", signup_form(next_url=next_url), user=request.state.user))
...
async def signup_post(
    ...
    if errors:
        return HTMLResponse(page_shell("Sign Up", signup_form(" ".join(errors), values, next_url=next_url),
                                       user=request.state.user))

    # Check if email already exists
    existing = await get_user_by_email(db, email)
    if existing:
        return HTMLResponse(page_shell("Sign Up", signup_form("An account with that email already exists.", values, next_url=next_url),
                                       user=request.state.user))
