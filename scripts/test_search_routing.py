...
TEST_CASES: list[tuple[str, str]] = [
    ...
    # ── probe 112 remote + SDK tools (merged from parallel session): heartbeat / trust / tcp / fern / liblab ──
    ...
    ("fern sdk alternative", "api"),                       # bare "fern"→api (Fern SDK generation)
    ("liblab alternative", "api"),                         # bare "liblab"→api
    # Regressions: zero-trust/network-monitoring/autocomplete-frontend unchanged
    ...
    # ── probe 117: breaking-change / deprecation / changelog-tool dead zones ──
    # Dead zones fixed: "breaking change detector" → RAW_FIRST; "backwards compatible" → RAW_FIRST;
    # "deprecation detection"/"deprecation warning" → RAW_FIRST; "announcekit/headway/launchnotes" → RAW_FIRST;
    # "optic api diff" → RAW_FIRST.
    ("breaking change detector", "api"),               # bigram "breaking change"→api
    ("breaking changes api", "api"),                   # bigram "breaking changes"→api (plural form)
    ("detect breaking changes", "api"),                # bigram "breaking changes"→api
    ("backward compatibility check", "api"),           # bare "backward"→api
    ("backward compat library", "api"),                # bare "backward"→api
    ("backwards compatible library", "api"),           # bare "backwards"→api
    ("backwards breaking change", "api"),              # bare "backwards"→api
    ("optic api diff", "api"),                         # bare "optic"→api (Optic API diff tool)
    ("optic alternative openapi", "api"),              # bare "optic"→api
    ("deprecation warning library", "developer"),      # bare "deprecation"→developer
    ("deprecation detection tool", "developer"),       # bare "deprecation"→developer
    ("deprecated api scanner", "developer"),           # bare "deprecated"→developer
    ("deprecated function warning", "developer"),      # bare "deprecated"→developer
    ("announcekit alternative", "feedback"),           # bare "announcekit"→feedback
    ("headway changelog widget", "feedback"),          # bare "headway"→feedback
    ("launchnotes alternative", "feedback"),           # bare "launchnotes"→feedback
    # Regressions: existing entries unchanged
    ("crypto library nodejs", "security"),             # bare "crypto"→security unchanged (node:crypto)
    ("api versioning tool", "api"),                    # bare "api"→api (api versioning still api-tools)
    ("release notes widget", "feedback"),              # bigram "release notes"→feedback unchanged
    ("changelog generator", "devops"),                 # bare "changelog"→devops unchanged
