...
def draft_posts(top_commits: list[dict], lessons: list[str], gotchas: list[str]) -> list[str]:
    ...
    if fix_commits:
        ...
        post = f"Lesson learned building IndieStack: {gotcha}"
        if len(post) <= 280:
            posts.append(post)
    ...
    if len(top_commits) >= 3:
        ...
        if parts:
            ...
            post = f"Last {len(top_commits)} commits on IndieStack: {summary}. Building in public. {SITE_URL}"
            if len(post) <= 280:
                posts.append(post)
