...
VALID_SHORT_NAMES = {
    ...
    "standard",     # AI Standards & Specs (LIKE '%standard%' ✓)
}
...
def main():
    ...
    # ── 4. Multi-token keys (can never fire — router only builds bigrams) ───
    # Keys with 3+ space-separated tokens will never match any query.
    ...
    stop_match = re.search(r'_FTS_STOP_WORDS = \{(.*?)\}', db_text, re.DOTALL)
    stop_words: set[str] = set()
    if stop_match:
        stop_words = set(re.findall(r"'([^'\s][^']*[^'\s]|[^'\s])'", stop_match.group(1)))
